...
module.exports = function(eleventyConfig) {
  ...
  eleventyConfig.addCollection("sculptures", function(collection) {
    return collection.getAllSorted().filter(function(item) {
      return item.inputPath.match(/^\.\/_src\/sculptures\//) !== null;
    });
  });

  // only content in the `recipes` directory
  eleventyConfig.addCollection("truncsculptures", function(collection) {
    let i = 0;
    return collection.getAllSorted().reverse().filter(function(item) {
      if( item.data.feat != true && item.data.subfeat != true && i <= 9 &&  item.inputPath.match(/^\.\/_src\/sculptures\//) !== null ) {
        i++;
        return item;
      }
    });
